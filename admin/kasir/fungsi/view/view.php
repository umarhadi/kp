...
class view
{
	...
	function penjualan()
	{
		$sql = "SELECT penjualan.* , barang.id_barang, barang.nama_barang, barang.merk, barang.harga_jual, barang.satuan_barang, member.id_member,
	...
	function penjualan_online(){
		$sql = "SELECT penjualan.* , barang.id_barang, barang.nama_barang, barang.merk, barang.harga_jual, barang.satuan_barang, member.id_member,
						member.nm_member from penjualan 
					   left join barang on barang.id_barang=penjualan.id_barang 
					   left join member on member.id_member=penjualan.id_member
					   WHERE penjualan.jenis = 'Online' ORDER BY id_penjualan";
		$row = $this->db->prepare($sql);
		$row->execute();
		$hasil = $row->fetchAll();
		return $hasil;
	...
	function penjualan_bulan_row()
	{
		$sql = "SELECT nota.* , barang.id_barang, barang.nama_barang, barang.harga_beli, member.id_member,
						member.nm_member from nota 
					   left join barang on barang.id_barang=nota.id_barang 
					   left join member on member.id_member=nota.id_member WHERE nota.periode = '02-2021' ORDER BY id_nota ASC";
		$row = $this->db->prepare($sql);
		$row->execute();
		$hasil = $row->fetch();
		return $hasil;
